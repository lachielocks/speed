...
bkcore.hexgl.Gameplay = function (opts) {
  ...
  this.lastCoinUpdate = 0; // Track when we last updated the coin counter
  ...
  this.modes.timeattack = function () {
    ...
    if (
      cp == self.track.checkpoints.start &&
...
bkcore.hexgl.Gameplay.prototype.update = function () {
  ...
  if (
    ...
    // Update coin counter every 10 seconds, but only if the vehicle is moving
    if (this.shipControls) {
      // Check if the ship is moving using speed ratio (0-1)
      var speedRatio = this.shipControls.getSpeedRatio();
      var isMoving = speedRatio > 0.01; // 1% of max speed threshold
      var currentTime = this.timer.time.elapsed / 1000; // Convert to seconds

      // Debug: Uncomment to see the speed ratio in console
      // if (this.debugCounter++ % 60 === 0) console.log('Speed Ratio:', speedRatio, 'Moving:', isMoving);

      if (isMoving && currentTime - this.lastCoinUpdate >= 10) {
        this.lastCoinUpdate = currentTime;
        if (this.hud) {
          this.hud.coinCount++;
          this.hud.updateCoins(this.hud.coinCount);
        }
      }
    }
