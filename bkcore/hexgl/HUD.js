...
bkcore.hexgl.HUD = function (opts) {
  ...
  this.time = "";
  this.coinCount = 0;
  this.coinFontRatio = 20; // Font size (smaller number = larger text)
  this.coinMarginRatio = 30; // Distance from left edge
  this.coinYRatio = 10; // Distance from top
...
bkcore.hexgl.HUD.prototype.display = function (msg, duration) {
  ...
  this.messageDuration =
    duration == undefined ? this.messageDurationD : duration * 60;
};
...
bkcore.hexgl.HUD.prototype.updateCoins = function (coins) {
  this.coinCount = coins;
};
bkcore.hexgl.HUD.prototype.resetCoins = function () {
  this.coinCount = 0;
};
...
bkcore.hexgl.HUD.prototype.update = function (
  speed,
  speedRatio,
  shield,
  shieldRatio,
  ...
  if (this.step == 0) {
    ...
    if (this.coinCount >= 0) {
      ...
      this.ctx.font = SCREEN_WIDTH / this.coinFontRatio + "px " + this.font;
      this.ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
      this.ctx.fillText(
        "Coins: " + this.coinCount,
        SCREEN_WIDTH / this.coinMarginRatio, // X position (from left)
        SCREEN_HEIGHT / this.coinYRatio, // Y position (from top)
      );
      this.ctx.restore(); // Restore previous context state (which includes textAlign: 'center')
    }
    ...
    if (this.previousMessage != "") {
      ...
      this.ctx.font =
        SCREEN_WIDTH / this.messageFontRatioEnd + "px " + this.font;
    ...
    if (this.message != "") {
      if (this.messageTiming < this.messageDurationS) {
        this.messageAS +=
          (this.messageFontAlpha - this.messageAS) * this.messageFontLerp;
